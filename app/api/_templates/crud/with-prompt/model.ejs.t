...
<% for (let i=0; i<relationships.length; i++) { -%>
  ...
  <% } else if (relationships[i][0] === "manytomany") { -%>  
    <%= capitalizedName %>.belongsToMany(<%= h.inflection.capitalize(relationships[i][1]) %> , { through: '<%= pluralName %>_<%=h.inflection.pluralize(relationships[i][1])%>' })
    <%= h.inflection.capitalize(relationships[i][1]) %>.belongsToMany(<%= capitalizedName %>, { through: '<%= pluralName %>_<%=h.inflection.pluralize(relationships[i][1])%>' })
  <% } -%>
<% } -%>
